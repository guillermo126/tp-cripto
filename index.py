    ...
    if len(text.get()) != 0:
        if text.get().endswith(".bmp"):
            if selec.get() == 1:
                ...
                input_image = BmpImagePlugin.BmpImageFile(text.get())
                ...
                output_image.save("encriptado.bmp")
                messagebox.showinfo("Exito", "Ha finalizado la encriptación")

            else:
                keys = hashlib.md5(b"aaaabbbb").digest()
                output_image = BmpImagePlugin.BmpImageFile(text.get())
                output_data = output_image.tobytes()

                output_data = bytes(b ^ a for a, b in zip(output_data, cycle(keys)))

                output_decryptimage = output_image.copy()
                output_decryptimage.frombytes(output_data)
                output_decryptimage.save("desencriptado.bmp")
                messagebox.showinfo("Exito", "Ha finalizado la desencriptación")
        else:
            messagebox.showinfo(
                "Error",
...
def encriptarImagen(ruta, salida):
    file = open(ruta, "rb")
    ...
    while (offset < size):
        ...
        if (offset + block_size < size):
            ...
            offsetEncrypted = offsetEncrypted + block_size
        else:
            fileEncrypted.close()
            file.close()
            break
...
def desencriptarImagen(entrada, salida):
    ...
    while offset < size:
        ...
        leerBloque = file.read(block_size)
        cifrarBloque = int.from_bytes(leerBloque, "big")
        bloqueCifrado = cipher.decrypt(cifrarBloque)
        bloquedecifrado = bloqueCifrado.to_bytes(block_size, byteorder="big")
        fileEncrypted.seek(offsetEncrypted)

        fileEncrypted.write(str(bloquedecifrado))
